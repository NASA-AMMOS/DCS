...
public class KmcSdlsEngine {
    ...
    public String getCryptoLibErrorCodeEnum(int errorCode) {
        return kmc_sdls.sdls_get_error_code_enum_string(errorCode);
    }
    ...
    private void configureKmcSdlsEngineFromProperties(Properties props) throws Exception {
        ...
        String tcCreateEcf = props.getProperty("cryptolib.tc.create_ecf");
        if (tcCreateEcf != null) {
            cryptoCreateFecf = Boolean.parseBoolean(tcCreateEcf);
        }

        boolean processSdlsPdus = Boolean.parseBoolean(props.getProperty("cryptolib.process_tc.process_pdus"));
        String tcProcessSdlsPdus = props.getProperty("cryptolib.tc.process_pdus");
        if (tcProcessSdlsPdus != null) {
            processSdlsPdus = Boolean.parseBoolean(tcProcessSdlsPdus);
        }
        ...
        String tcIgnoreSaState = props.getProperty("cryptolib.tc.ignore_sa_state");
        if (tcIgnoreSaState != null) {
            ignoreSaState = Boolean.parseBoolean(tcIgnoreSaState);
        }

        boolean ignoreAntiReplay = Boolean.parseBoolean(props.getProperty("cryptolib.process_tc.ignore_antireplay"));
        String tcIgnoreAntiReplay = props.getProperty("cryptolib.tc.ignore_antireplay");
        if (tcIgnoreAntiReplay != null) {
            ignoreAntiReplay = Boolean.parseBoolean(tcIgnoreAntiReplay);
        }
        ...
        String tcCryptoCheckFecf = props.getProperty("cryptolib.tc.check_ecf");
        if (tcCryptoCheckFecf != null) {
            cryptoCheckFecf = Boolean.parseBoolean(tcCryptoCheckFecf);
        }
    ...
    public int configureCryptoLib(sadbType sadbTypeEnum, cryptographyType cryptographyTypeEnum, boolean cryptoCreateFecf, boolean processSdlsPdus,
        ...
        int status = kmc_sdls.sdls_config_cryptolib(sadb_type, cryptography_type);
        if (status != KMC_ENGINE_SUCCESS) {
            throw new Exception("Unable to Configure CryptoLib Core, Error Code: " + status + ", Error Message: " + kmc_sdls.sdls_get_error_code_enum_string(status));
        }
        this.engineStatus = status;

        status = kmc_sdls.sdls_config_cryptolib_tc(crypto_create_fecf, process_sdls_pdus, has_pus_hdr,
                ignore_anti_replay, ignore_sa_state, unique_sa_per_mapid, crypto_check_fecf, vcid_bitmask,
    ...
    public int addGvcidManagedParameter(int transferFrameVersionNumber, int spacecraftId, int virtualChannelID, boolean hasFecf, boolean hasSegmentationHdr, int maxTcFrameSize)
        ...
        int status = kmc_sdls.sdls_config_add_gvcid_managed_parameter_tc(tfvn, scid, vcid, has_fecf, has_segmentation_hdr, max_tc_frame_size);
        this.engineStatus = status;

        if (status != KMC_ENGINE_SUCCESS)
            throw new Exception("Unable to Configure CryptoLib GVCID Managed Parameter, Error Code: " + status + ", Error Message: " + kmc_sdls.sdls_get_error_code_enum_string(status));
    ...
    public int configureMariaDb(String mysqlHostname, String mysqlDatabase, int mysqlPort, boolean mysqlRequireSecureTransport, boolean mysqlTlsVerifyServer, String mysqlTlsCa, String mysqlTlsCapath, String mysqlMtlsCert, String mysqlMtlsKey, String mysqlMtlsClientKeyPassword, String mysqlUsername, String mysqlPassword)
        ...
        int status = kmc_sdls.sdls_config_mariadb(mysqlHostname, mysqlDatabase, mysql_port, mysql_require_secure_transport, mysql_tls_verify_server, mysqlTlsCa, mysqlTlsCapath, mysqlMtlsCert, mysqlMtlsKey, mysqlMtlsClientKeyPassword, mysqlUsername, mysqlPassword);
        this.engineStatus = status;

        if (status != KMC_ENGINE_SUCCESS)
            throw new Exception("Unable to Configure MariaDB, Error Code: " + status + ", Error Message: " + kmc_sdls.sdls_get_error_code_enum_string(status));
    ...
    public int configureCryptoService(String protocol, String kmcCryptoHostname, int kmcCryptoPort,
        ...
        int status = kmc_sdls.sdls_config_kmc_crypto_service(protocol, kmcCryptoHostname, kmc_crypto_port,
        ...
        if (status != KMC_ENGINE_SUCCESS)
            throw new Exception("Unable to Configure KMC Crypto Service, Error Code: " + status + ", Error Message: " + kmc_sdls.sdls_get_error_code_enum_string(status));
    ...
    public int configureCam(boolean camEnabled, String cookieFilePath, String keytabFilePath,
        ...
        int status = kmc_sdls.sdls_config_cam(cam_enabled, cookieFilePath, keytabFilePath,
    ...
    public int init() throws Exception {
        int status = kmc_sdls.sdls_init();
        this.engineStatus = status;

        if (status != KMC_ENGINE_SUCCESS)
            throw new Exception("Unable to Init KMC SDLS Engine, Error Code: " + status + ", Error Message: " + kmc_sdls.sdls_get_error_code_enum_string(status));

        return status;
    }

    public void shutdown() {
        kmc_sdls.sdls_shutdown();
    }
    ...
    public String applySecurity(String unencryptedFrameHexbytesString, String camCookies) throws Exception {
        ...
        SWIGTYPE_p_unsigned_short len_encrypted_frame = kmc_sdls.create_uint16t_ptr(0);

        SWIGTYPE_p_unsigned_char unencrypted_frame = kmc_sdls.hexstring_to_bytearray(unencryptedFrameHexbytesString, unencryptedFrameHexbytesString.length());
        SWIGTYPE_p_p_unsigned_char encrypted_frame = kmc_sdls.create_bytearray_pp();
        int status = kmc_sdls.apply_security_tc_cam(unencrypted_frame, bytes_len, encrypted_frame, len_encrypted_frame, camCookies);
        if (status != KMC_ENGINE_SUCCESS)
            throw new Exception("Unable to Apply Security on TC Frame, Error Code: " + status + ", Error Message: " + kmc_sdls.sdls_get_error_code_enum_string(status));

        String encrypted_frame_hexstring = kmc_sdls.bytearray_pp_to_hexstring(encrypted_frame, kmc_sdls.deref_uint16t_ptr(len_encrypted_frame));
    ...
    public SDLS_TC_TransferFrame processSecurity(String encryptedFrameHexbytesString, String camCookies) throws Exception {
        ...
        SWIGTYPE_p_int len_encrypted_frame = kmc_sdls.create_int_ptr(bytes_len);
        SWIGTYPE_p_unsigned_char encrypted_frame = kmc_sdls.hexstring_to_bytearray(encryptedFrameHexbytesString, encryptedFrameHexbytesString.length());
        this.tc_ptr = new TC_t();

        int status = kmc_sdls.process_security_tc_uint8t(encrypted_frame, len_encrypted_frame, this.tc_ptr, camCookies);
        if (status != KMC_ENGINE_SUCCESS)
            throw new Exception("Unable to Process Security on TC Frame, Error Code: " + status + ", Error Message: " + kmc_sdls.sdls_get_error_code_enum_string(status));
        ...
        //sdlsTCFrame.tfvn = kmc_sdls.get_tfvn(this.tc_ptr);
        //sdlsTCFrame.bypass = kmc_sdls.get_bypass(this.tc_ptr);
        //sdlsTCFrame.cc = kmc_sdls.get_cc(this.tc_ptr);
        //sdlsTCFrame.spare = kmc_sdls.get_spare(this.tc_ptr);
        //sdlsTCFrame.scid = kmc_sdls.get_scid(this.tc_ptr);
        //sdlsTCFrame.vcid = kmc_sdls.get_vcid(this.tc_ptr);
        //sdlsTCFrame.fl = kmc_sdls.get_fl(this.tc_ptr);
        //sdlsTCFrame.fsn = kmc_sdls.get_fsn(this.tc_ptr);
        ...
        sdlsTCFrame.iv = kmc_sdls.bytearray_to_hexstring(iv_ptr, sdlsTCFrame.iv_field_len);
        ...
        sdlsTCFrame.sn = kmc_sdls.bytearray_to_hexstring(sn_ptr, sdlsTCFrame.sn_field_len);
        ...
        sdlsTCFrame.pad = kmc_sdls.bytearray_to_hexstring(pad_ptr, sdlsTCFrame.pad_field_len);
//        sdlsTCFrame.sh = kmc_sdls.get_sh(this.tc_ptr);
//        sdlsTCFrame.spi = kmc_sdls.get_spi(this.tc_ptr);
//        sdlsTCFrame.iv_field_len = kmc_sdls.get_iv_field_len(this.tc_ptr);
//        SWIGTYPE_p_unsigned_char iv_ptr = kmc_sdls.get_iv_ptr(this.tc_ptr);
//        sdlsTCFrame.iv = kmc_sdls.bytearray_to_hexstring(iv_ptr,sdlsTCFrame.iv_field_len);
//        sdlsTCFrame.sn_field_len = kmc_sdls.get_sn_field_len(this.tc_ptr);
//        SWIGTYPE_p_unsigned_char sn_ptr = kmc_sdls.get_sn_ptr(this.tc_ptr);
//        sdlsTCFrame.sn = kmc_sdls.bytearray_to_hexstring(sn_ptr,sdlsTCFrame.sn_field_len);
//        sdlsTCFrame.pad_field_len = kmc_sdls.get_pad_field_len(this.tc_ptr);
//        SWIGTYPE_p_unsigned_char pad_ptr = kmc_sdls.get_pad_ptr(this.tc_ptr);
//        sdlsTCFrame.pad = kmc_sdls.bytearray_to_hexstring(pad_ptr,sdlsTCFrame.pad_field_len);
        // Parse Frame Data
        sdlsTCFrame.tc_pdu_len = this.tc_ptr.getTc_pdu_len();
        SWIGTYPE_p_unsigned_char pdu_ptr = this.tc_ptr.getTc_pdu();
        sdlsTCFrame.tc_pdu = kmc_sdls.bytearray_to_hexstring(pdu_ptr, sdlsTCFrame.tc_pdu_len);
//        sdlsTCFrame.tc_pdu_len = kmc_sdls.get_tc_pdu_len(this.tc_ptr);
//        SWIGTYPE_p_unsigned_char pdu_ptr = kmc_sdls.get_pdu_ptr(this.tc_ptr);
//        sdlsTCFrame.tc_pdu = kmc_sdls.bytearray_to_hexstring(pdu_ptr,sdlsTCFrame.tc_pdu_len);
        ...
        sdlsTCFrame.mac = kmc_sdls.bytearray_to_hexstring(mac_ptr, sdlsTCFrame.mac_field_len);
        sdlsTCFrame.fecf = this.tc_ptr.getTc_sec_trailer().getFecf();
//        sdlsTCFrame.mac_field_len = kmc_sdls.get_mac_field_len(this.tc_ptr);
//        SWIGTYPE_p_unsigned_char mac_ptr = kmc_sdls.get_mac_ptr(this.tc_ptr);
//        sdlsTCFrame.mac = kmc_sdls.bytearray_to_hexstring(mac_ptr,sdlsTCFrame.mac_field_len);
//        sdlsTCFrame.fecf = kmc_sdls.get_fecf(this.tc_ptr);
    ...
    public String processSecurityReturnDataOnly(String encryptedFrameHexbytesString, String camCookies) throws Exception {
        ...
        return kmc_sdls.bytearray_to_hexstring(this.tc_ptr.getTc_pdu(), this.tc_ptr.getTc_pdu_len());
    }
